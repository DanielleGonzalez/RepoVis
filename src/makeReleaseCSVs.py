...
PROGRAM OUTPUT: 
	...
	2. 3 csv files for the visualizations I want to do  with Google Charts
'''

def writeDataToFiles(data, releasesFile, prFile, issueFile,contribFile):

	# RELEASES FILE
	rFile = open(releasesFile, "a", encoding="utf-8")
	singlerelease = next(iter(data.values())) #get any dict value to get the attributes
	try:
		# Create the header with the attribute names
		for attrib in singlerelease:
			rFile.write(str(attrib) + ",")
		rFile.write('\n')
		# Add data to the CSV
		for record in data:
			row = data[record]
			for attribute in row:
				#print(row[attribute])
				rFile.write(str(row[attribute]) + ",")
			rFile.write("\n")
	except Exception as exc:
		print("Generated an exception during Releases output: " + str(exc))
	else:
		print("Release Data has been output to " + str(releasesFile))
		rFile.close()

	# PR FILE
	pFile = open(prFile, "a", encoding="utf-8")
	try:
		# Create the header with the attribute names
		pFile.write("releaseName,releaseDate,prsOpened,prsClosed")
		pFile.write('\n')
		# Add data to the CSV
		for record in data:
			row = data[record]
			pFile.write(str(row['Release']) + "," +str(row['ReleaseDate']) + ","+ str(row['prsOpened']) + "," + str(row['prsClosed']) + "\n")
	except Exception as exc:
		print("Generated an exception during PR output: " + str(exc))
	else:
		print("PR Data has been output to " + str(prFile))
		pFile.close()
	
	# ISSUE FILE
	iFile = open(issueFile, "a", encoding="utf-8")
	try:
		# Create the header with the attribute names
		iFile.write("releaseName,releaseDate,prsOpened,prsClosed")
		iFile.write('\n')
		# Add data to the CSV
		for record in data:
			row = data[record]
			iFile.write(str(row['Release']) + "," + str(row['ReleaseDate']) + "," + str(row['issuesOpened']) + "," + str(row['issuesClosed']) + "\n")
	except Exception as exc:
		print("Generated an exception during Issues output: " + str(exc))
	else:
		print("Issue Data has been output to " + str(issueFile))
		iFile.close()
	
	# CONTRIBUTOR FILE
	cFile = open(contribFile, "a", encoding="utf-8")
	try:
		# Create the header with the attribute names
		cFile.write("releaseName,releaseDate,contributors")
		cFile.write('\n')
		# Add data to the CSV
		for record in data:
			row = data[record]
			cFile.write(str(row['Release']) +  "," + str(row['ReleaseDate']) + "," + str(row['contributors']) + "\n")
	except Exception as exc:
		print("Generated an exception during Contributor output: " + str(exc))
	else:
		print("Issue Data has been output to " + str(contribFile))
		cFile.close()
...
def countByRelease(releases,pullRequests,issues,repoName):
	...
	prFile = "../data/" + str(repoName) + "-prByRelease" + ".csv"
	issueFile = "../data/" + str(repoName) + "-issueByRelease" + ".csv"
	contribFile = "../data/" + str(repoName) + "-contribByRelease" + ".csv"

	writeDataToFiles(finalReleaseData, outputPath,prFile,issueFile,contribFile)
